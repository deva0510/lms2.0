...
def assign_subjects_classes(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'admin-template/assign_subjects_classes.html', {'teachers': teachers, 'subjects': subjects, 'classes': classes,'message':message,'k':k})
...
def addsubject(request):
    # Assuming you have a logged-in user, you can get the school_id associated with the user
    k=MenuItem.objects.filter(parent_category=None)  
    sch = Schools.objects.filter(usernumber=request.user.id).first()
    message = ''
